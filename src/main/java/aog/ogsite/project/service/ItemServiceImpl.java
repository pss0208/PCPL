...
import aog.ogsite.project.repository.BasketRepository;
...
//        int homeEndPage = Math.min(itemBoards.getTotalPages(), itemBoards.getPageable().getPageNumber() + 3);
    ...
    @Override
    public ItemPageDto getItemPagingDtoBySale(Pageable pageable, String firstCategory) {
        ItemPageDto itemPageDto = new ItemPageDto();

        Page<ItemDto> itemBoards = itemRepository.findSaleItem(pageable, firstCategory);
//        int homeStartPage = Math.max(1, itemBoards.getPageable().getPageNumber() - 1);
        int homeStartPage = 1;
//        int homeEndPage = Math.min(itemBoards.getTotalPages(), itemBoards.getPageable().getPageNumber() + 3);
        int homeEndPage = 2;
    ...
    public List<Item> getMainCarouselItemList() {
        ...
        Item firstItem = itemRepository.findByItemIdxAndColorAndRep(49L, "화이트", true); // 반팔
        Item secondItem = itemRepository.findByItemIdxAndColorAndRep(95L, "블랙", true); // 반바지
        Item thirdItem = itemRepository.findByItemIdxAndColorAndRep(96L, "네이비", true); // bottom / cotton
        Item fourthItem = itemRepository.findByItemIdxAndColorAndRep(4L, "블랙", true);
        Item fifthItem = itemRepository.findByItemIdxAndColorAndRep(6L, "블랙", true);
        // 원하는 사진 D:\ogsite\src\main\resources\static\image\etc\main_carousel 경로에 넣고 해당 상품 idx, 컬러 맞추면 됨.
